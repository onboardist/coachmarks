...
/* src/components/ActionButton.svelte generated by Svelte v2.7.0 */
function type(_ref) {
	var coachmark = _ref.coachmark;
...
var methods = {
	handleClick: function handleClick() {
		var coachmark = this.get().coachmark;
		if (!coachmark) {
		...
		if (flow) {
			var next = flow.getNext(coachmark.name);
			if (next) {
				this.type = 'next';
				return this.fire('next', { next: next });
...
assign(ActionButton.prototype, methods);

ActionButton.prototype._recompute = function _recompute(changed, state) {
	if (changed.coachmark) {
		if (this._differs(state.type, state.type = type(state))) changed.type = true;
	}
};
...
assign(Text.prototype, methods$1);
...
function create_main_fragment$2(component, ctx) {
	...
	return {
		...
		d: function d(detach) {
			...
			if (component.refs.glow === div_4) component.refs.glow = null;
		}
	};
}
...
assign(Overlay.prototype, methods$2);
...
function coach(mark) {
  ...
  var overlay = cache.default('overlay', function () {
    return new Overlay({ target: body });
  });
  overlay.show(elm);
  ...
  var actionBtn = cache.default('actionButton', function () {
    return new ActionButton({
      target: body
    });
  });
  actionBtn.set({ coachmark: mark });
  actionBtn.on('clear', function () {
    return clear();
  });
  actionBtn.on('next', function (event) {
    return draw(event.next);
  });
...
//# sourceMappingURL=coachmarks.esm.js.map
